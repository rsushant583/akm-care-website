...
function ServicesTab({ services, onSave, onDelete }: any) {
  ...
  return (
    <div className="space-y-4">
      ...
      <select className="w-full px-4 py-3 rounded-xl border" value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })}>{["Briefcase","GraduationCap","Users","ShieldCheck","Truck","ChefHat","Plane","Wrench","Settings","Heart"].map((x) => <option key={x}>{x}</option>)}</select>
