/** Provider-neutral shipping status + fulfillment coupling. Pure functions for unit tests. */
...
/** Projection statuses written to public.shipping (customer/admin safe). */
...
/**
 * Monotonic shipping transition: never regress along the happy path.
 * Terminal states (cancelled/failed/rto) only from pre-delivery states.
 */
export function canAdvanceShippingStatus(from: string, to: string): boolean {
  ...
  if (f === "delivered") {
    return t === "rto"; // only RTO after delivered
  }
  if (f === "cancelled" || f === "failed") return false;
  if (f === "rto") return false;
...
/** Map Shiprocket current_status / labels → AKM shipping status. Unknown → null. */
...
/**
 * Suggested fulfillment status from shipping status.
 * Caller must apply only via order_status_transition_allowed and never regress.
 */
...
/** Never regress fulfillment (e.g. delivered → shipped). */
